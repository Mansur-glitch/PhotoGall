...
#define MISS_DEFINITION(def) \
  static_assert(false, "Missing definition: " STRING(def));
...
class Settings : public QObject {
 ...
 public:
  ...
  Q_INVOKABLE Qt::Orientation getLayoutOrientation() const noexcept {
    ...
    return QVariant(m_settings.value("layoutOrientation"))
  ...
  Q_INVOKABLE QString getStartDirectory() const noexcept {
    if (!m_settings.contains("startDirectory")) {
      return QDir::homePath();
    }
    return m_settings.value("startDirectory").toString();
  }

  Q_INVOKABLE void setStartDirectory(QString dir) {
    m_settings.setValue("startDirectory", dir);
  }
...
int main(int argc, char** argv) {
  try {
    ...
    qmlRegisterType<PictureListModel>("localhost.PictureModel", 1, 0,
                                      "PictureListModel");
    qmlRegisterType<CollectionManager>("localhost.PictureModel", 1, 0,
                                       "PictureCollection");
    qmlRegisterType<PictureProvider>("localhost.PictureModel", 1, 0,
                                     "PictureProvider");

    qmlRegisterType<DirectoryTreeModel>("localhost.DirectoryTreeModel", 1, 0,
                                        "DirectoryTreeModel");
    qmlRegisterType<DirectoryValidator>("localhost.Utility", 1, 0,
                                        "DirectoryValidator");
    qmlRegisterType<StrictIntValidator>("localhost.Utility", 1, 0,
                                        "StrictIntValidator");
    ...
    qmlRegisterSingletonInstance("localhost.Utility", 1, 0, "LoseFocusDetector",
                                 LoseFocusDetector::s_singletonInstance);
    qmlRegisterSingletonInstance("localhost.Utility", 1, 0, "Settings",
                                 &settings);
