...
requires std::ranges::view<ViewTy> &&
requires (ViewTy v) {
...
{
public:
  Group(uint32_t id, uint32_t sampleIndex, uint32_t begin = 0, uint32_t size = 1);
...
public:
  ...
  void clear();
private:
  uint32_t filterRange(uint32_t begin, uint32_t end,
...
public:
  ...
  public:
    ...
    uint32_t getIdAt(uint32_t num) const;
    std::optional<uint32_t> getIdBySample(uint32_t sample,
                                           const Predicate2<uint32_t>& sameGroupPred);
    // Side effect: expands m_groups, writes to data
    void arrange(uint32_t begin, uint32_t size,
               const Predicate2<uint32_t>& sameGroupPred,
  ...
  private:
    ...
    void arrange_distributeNewData(QList<uint32_t>& currentMarkup, uint32_t newDataSize,
                                   const Predicate2<uint32_t>& sameGroupPred);
    void arrange_sortGroups(const Predicate2<uint32_t>&);
    // Temporary set local group order to main layout's field. Used in write data.
...
{
public:
  CollectionView(QList<uint32_t>& indexes, const GroupLayoutBase* inputLayout = nullptr);
...
{
public:
  NullView(QList<uint32_t>& indexes, const GroupLayoutBase* inputLayout = nullptr);
...
public:
  GroupingView(Predicate2<uint32_t> sameGroupPred,
    ...
    // m_sameGroupPred = [this, sameGroupPred](uint32_t index1, uint32_t index2)
    //   {
    //     return sameGroupPred(m_source->get<ElementTy>()[index1],
    //                          m_source->get<ElementTy>()[index2]);
    //   };

    // m_lessGroupPred = [this, lessGroupPred](const Group& lhs, const Group& rhs)
    //   {
    //     const auto& left = m_source->get<ElementTy>()[lhs.getSample()];
    //     const auto& right = m_source->get<ElementTy>()[rhs.getSample()];
    //     return lessGroupPred(left, right);
    //   };
...
public:
  SortingView(Predicate2<uint32_t> elementLessPred,
               QList<uint32_t>& indexes,
               const GroupLayoutBase* inputLayout = nullptr);
    // m_byIndexLessCompare = [this, elementLessPred](const uint32_t& lhs, const uint32_t& rhs)
    //   {
    //     return elementLessPred(m_source->get<ElementTy>()[lhs],
    //                            m_source->get<ElementTy>()[rhs]);
    //   };
  virtual void update() override;
  virtual void reset() override;
private:
  void sortPartiallySorted(uint32_t begin, uint32_t end, uint32_t unsortedBegin);
...
public:
  FilteringView(Predicate1<uint32_t> filterPred,
                QList<uint32_t>& indexes,
               const GroupLayoutBase* inputLayout = nullptr);
    // m_filterByIndex = [this, filterPred](const uint32_t& index)
    //   {
    //     return filterPred(m_source->get<ElementTy>()[index]);
    //   };
  virtual void update() override;
  virtual void reset() override;
  const FilteredGroupLayout* outputLayout() const override;
protected:
  FilteringView(QList<uint32_t>& indexes, const GroupLayoutBase* inputLayout = nullptr);
...
{
public:
  RemovingView(QList<uint32_t>& indexes, const QBitArray& deletedMask,
               const GroupLayoutBase* inputLayout = nullptr);
};
...
public:
  ...
  {
  public:
    DisplayGroup(Group g)
    : Group(g)
    {}

    // const ElementTy& getItem(uint32_t index) const
    // {
    //   return (*m_data)[(*m_indexes)[begin() + index]];
    // }
  ...
  void setGrouping(uint32_t groupingNum, PredTy sameGroupPred, PredTy groupLessPred)
  {
    ...
    m_views[viewId].reset(new GroupingView(m_source.getIndexPredicate2(sameGroupPred),
  ...
  {
    const GroupLayoutBase* inputLayout = m_views[sortingViewId - 1]->outputLayout();
  ...
  void setFiltering(uint32_t filterNum, PredTy filter)
  {
    ...
    m_views[viewId].reset(new FilteringView(m_source.getIndexPredicate1(filter), m_source.getIndexes()));
